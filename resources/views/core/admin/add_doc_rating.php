...
if (isset($_POST['add'])) {
    ...
    $rate = $_POST['rate'];
    $query = "INSERT INTO ratings (rate_id, doc_id, doc_name, rate) VALUES (?,?,?,?)";
    $stmt = $mysqli->prepare($query);
    $rc = $stmt->bind_param('ssss', $rate_id, $doc_id, $doc_name, $rate);
    $stmt->execute();
    if ($stmt) {
        $success = "Success" && header("refresh:1; url=manage_ratings.php");
    } else {
