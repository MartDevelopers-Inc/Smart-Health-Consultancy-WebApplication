...
if (isset($_POST['add'])) {
    ...
    $query = "INSERT INTO feedbacks (f_id, member_id, member_name, f_content) VALUES (?,?,?,?)";
    $stmt = $mysqli->prepare($query);
    $rc = $stmt->bind_param('ssss', $f_id, $member_id, $member_name, $f_content);
    $stmt->execute();
    if ($stmt) {
        $success = "Success" && header("refresh:1; url=manage_feedbacks.php");
    } else {
